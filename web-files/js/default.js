...
function avise_me_chegar(id, error, carregar, mail) {
    ...
    $.ajax({
        type: 'post',
        data: "EMAIL=" + email + "&REFERENCIA=" + referencia + "&CODQUANTIDADE=" + id,
        url: getMyFolderRoot() + '/pt/informacoes/avise-me-chegar',
        success: function (data) {
            swal({title: "Erro!", text: data, type: "error"});
//            $("." + error).html(data);
...
if (document.getElementById("checkout-panel-2") != null) {
...
function update_precocompra(nome, valor, qntdd, id) {
...
function update_precounitario(nome, valor, codquantidade) {
    ...
    $.ajax({
        ...
        success: function (data) {
            var cod = v.replace("#", "");
            var id = ".quantidade_" + codquantidade;
            var qntdd = $(id).val();
            
            update_precocompra("PRECOCOMPRA_" + cod, vl, qntdd, id);
            console.log(data);
        }
    });
}
...
function update_qntdd_estoque(codproduto,codquantidade, qntdd) {
    $.ajax({
        type: 'post',
        data: "codquantidade=" + codquantidade + "&quantidade=" + qntdd,
...
$('.ajax-form').submit(function (e) {
        ...
        if (!this.checkValidity()) {
            ...
            if (msg == '') {
                ...
                if (this.type == 'password') {
                    msg = "Por favor, verifique sua senha!";
                }
            }
        } else {
    ...
    if (!valid) {
        sweetAlert("Oops...", "Por favor, verifique seus dados!", "error");
        return;
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            swal({title: "Enviado!", text: "Cadastro feito com sucesso!", type: "success", confirmButtonText: "Ok"});
        }
    });
...
//            swal({title: "Pronto!", text: "Entrando no sistema...", type: "success", timer: 2000});
    
        },
        error: function (data, da , erro) {
            $btn.removeAttr('disabled');
            sweetAlert("Oops...", "Por favor, verifique seus dados!", "error");
        }
    });
...
function pOrdemUpdate(id, codproduto) {
    $("#" + id).addClass("Loader");
    var valor = $("#" + id).val();
    $.ajax({
        type: 'post',
        data: 'codproduto=' + codproduto + '&valor=' + valor.replace(/\,|\./, ""),
        url: getMyFolderRoot() + '/pt/produtos/update-ordem',
        success: function (data) {
            $("#" + id).removeClass("Loader");
        }
    });
...
function pOrdemFotoUpdate(id, codfoto) {
    $("#" + id).addClass("Loader");
    var valor = $("#" + id).val();
    $.ajax({
        type: 'post',
        data: 'codfoto=' + codfoto + '&valor=' + valor.replace(/\,|\./, ""),
        url: getMyFolderRoot() + '/pt/produtos/update-ordem-foto',
        success: function (data) {
            $("#" + id).removeClass("Loader");
        }
    });
...
function eQuantidadeEstoqueUpdate(id, codquantidade, codproduto, obj) {
    $("#" + id).addClass("Loader");
    var qntdd = $("#" + id).val();
    ...
    $.ajax({
        ...
        url: getMyFolderRoot() + '/pt/produtos/update-qntdd-estoque',
        success: function (data) {
            var valor = $("#pName_" + codquantidade).val();
            var vl = valor.replace(/\,|\./, "");
            $.ajax({
                type: 'post',
                ...
                url: getMyFolderRoot() + '/pt/produtos/update-precounitario',
                success: function (data) {
                    update_precocompra("PRECOCOMPRA_" + codproduto, vl, qntdd, obj.id);
                    $("#" + id).removeClass("Loader");
                    console.log(data);
                }
            });
...
function ePrecoUnitarioEstoqueUpdate(id,codquantidade, codproduto) {
    $("." + codproduto).addClass("Loader");
    var valor = $("#" + id).val();
    var vl = valor.replace(/\,|\./, "");
    $.ajax({
        type: 'post',
        ...
        url: getMyFolderRoot() + '/pt/produtos/update-precounitario',
        success: function (data) {
            var qntdd = $("#qName_" + codquantidade).val();
            update_precocompra("PRECOCOMPRA_" + codproduto, vl, qntdd,codquantidade);
            $("." + codproduto).removeClass("Loader");
            console.log(data);
        }
    });
}
