...
Class QuerysReload extends Model {
    ...
    public function qnts_de_produtos($client_hidden, $codproduto, $codquantidade) {
        $query = $this->db->prepare("SELECT SUM(QUANTIDADE) as QUANTIDADE  FROM `lista_desejos` WHERE CODPRODUTO='{$codproduto}' AND CLIENT_HIDDEN='{$client_hidden}' AND CODQUANTIDADE='{$codquantidade}'");
        $query->execute();
        if( $query->rowCount() ){
...
WHERE lista_desejos.CLIENT_HIDDEN='{$client_hidden}'");
     ...
     public function get_itens($client_hidden, $codquantidade) {
...
$qntdd = $model->qnts_de_produtos($client_hidden, $codproduto, $codquantidade);
...
$dados["CODQUANTIDADE"] = $codquantidade;

$model->add_lista_desejos($dados);    

echo $return["itens"] = $model->get_itens($client_hidden, $codquantidade);    
die();
