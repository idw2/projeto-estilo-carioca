...
GROUP BY ld.CODQUANTIDADE ORDER BY (q.ORDEM+0)");
    ...
    public function get_lista_desejos($client_hidden, Array $codquantidades) {
	...
	(
            ...
            (
                ...
                WHERE ld.CODQUANTIDADE='{$codquantidade}' AND CLIENT_HIDDEN='{$client_hidden}'
            )
	) 
	as PESO_TOTAL,
	...
	quantidades.CODQUANTIDADE,
	quantidades.ROTULO,
	quantidades.ESPECIFICACAO,
	quantidades.IS_COR,
	quantidades.COR,
	quantidades.ORDEM,
	(
...
INNER JOIN produtos_rel_quantidades ON produtos_rel_quantidades.CODPRODUTO=produtos.CODPRODUTO
INNER JOIN quantidades ON produtos_rel_quantidades.CODQUANTIDADE=quantidades.CODQUANTIDADE
WHERE lista_desejos.CLIENT_HIDDEN='{$client_hidden}' AND quantidades.CODQUANTIDADE='{$codquantidade}' 
...
ORDER BY (produtos.NOME) ASC");
    ...
    public function update_produtos($codquantidade, $quantidade) {
//        $query = $this->db->query("UPDATE produtos SET QUANTIDADE = (QUANTIDADE - {$quantidade}) WHERE CODPRODUTO='{$codproduto}'");
        $query = $this->db->query("UPDATE quantidades SET QUANTIDADE = (QUANTIDADE - {$quantidade}) WHERE CODQUANTIDADE='{$codquantidade}'");
        return true;
    }
...
/*
 * SETA O CODIGO DO ENDERECO DE ENTREGA
 */
$codendereco = trim($_POST['CODENDERECO']);
/*
 * SETA O CODIGO DE ALGUM BONUS EXISTENTE
 */
$bonus = trim($_POST["BONUS"]);
/*
 * SETA UMA CHAVE UNICA PARA O PEDIDO MO ARRAY DADOS
 */
$dados['CODPEDIDO'] = strtoupper(md5(uniqid(rand(), true)));
/*
 * SETA QUAL E O CLIENTE NO ARRAY DADOS
 */
$dados['CODCADASTRO'] = trim($_POST['CODCADASTRO']);

/*
 * SETA QUAL E A FORMA DE PAGAMENTO ESCOLHIDA PELO COMPRADOR
 */
$dados['FORMA_PGTO'] = trim($_POST['FORMA_PGTO']);
/*
 * SETA QUAL O NUMERO DO PEDIDO NO ARRAY DADOS
 */
$dados['N_PEDIDO'] = ((int) $model->get_num_pedidos() + 1);
/*
 * SETA QUAL A FORMA DE ENVIO NO ARRAY DADOS
 */
$dados['FORMA_ENVIO'] = trim($_POST['FORMA_ENVIO']);
/*
 * CARREGA O OBJETO CLIENTE COM OS DADOS DO COMPRADOR
 */
$cliente = $model->get_cadastro($dados['CODCADASTRO']);
/*
 * CARREGA TODAS OS CODIGOS DE QUANTIDADES EXISTENTES EM UM ARRAY
 */
$wl = $model->get_all_codquantidade($client_hidden);
if ($wl) {
...
if ($ticket != 0) {
    if ($ticket->TIPO == "aniversario") {
        #$t = $model->limpaValorReal(trim($_POST['TOTAL_PARCIAL']));
        $t = 0;

        $sem_desejos = $model->get_lista_desejos($client_hidden, $wl_arr);
        if ($sem_desejos) {
            foreach ($model->get_lista_desejos($client_hidden, $wl_arr) as $compraObj) {
                $t = ($t + $compraObj->TOTAL);
            }
        }
...
if ($sem_compras) {
    foreach ($sem_compras as $compra) {
        $model->update_produtos($compra->CODQUANTIDADE, $compra->QUANTIDADE);
    }
}
...
$quebra_linha = "\n";
$emailsender = "webmaster@estilocarioca.com.br";
$nomeremetente = "Estilo Carioca";
$comcopia = $email;
...
$emaildesitnatario = "webmaster@estilocarioca.com.br";
mail($emaildesitnatario, $assunto, $mensagemHTML, $headers, "-f" . $emailsender);
