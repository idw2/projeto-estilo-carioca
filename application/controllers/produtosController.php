...
//            $primeira_lista = $lista_desejos->get_drop_lista_desejos(null, CLIENT_HIDDEN);
    ...
    public function produtos_lista() {
        if ($this->permitir_acesso($_SESSION["EMAIL"], $_SESSION["SENHA"], "ADMINISTRADOR")) {
            ...
            $this->assign("qtdd_junior", $model->qnts_produtos_for_categoria("junior"));
            $this->assign("qtdd_nilton_santos", $model->qnts_produtos_for_categoria("nilton-santos"));
            $this->assign("total_produtos", (int) $model->qnts_produtos_for_categoria("junior") + (int) $model->qnts_produtos_for_categoria("nilton-santos"));
...
//            $this->assign("ERRO_NAO_EXISTE_SOLICITACAO", "ERRO_NAO_EXISTE_SOLICITACAO");
    ...
    public function enviar_aviso() {
        if ($this->permitir_acesso($_SESSION["EMAIL"], $_SESSION["SENHA"], "ADMINISTRADOR")) {
            ...
            $this->disparar_emailGoogleAppsHTML($emaildesitnatario, FALSE, $emailsender, $assunto, $mensagemHTML);
...
//            echo "<script>window.location='" . MEU_SITE . "produtos/produtos_lista'</script>";
                ...
                if ($de == "") {
                    ...
                    if ($model->insert_produto($dados)) {
                        ...
                        while ($i < 101) {
                            ...
                            $rotulo = "rotulo_{$i}";
                            $especificacao = "especificacao_{$i}";
                            $is_cor = "is_cor_{$i}";
                            $cor = "cor_{$i}";
                            ...
                            if ($_POST[$rotulo] != "") {
                                ...
                                $dados_rel_quantidade["CODQUANTIDADE"] = strtoupper(md5($this->getPrimarykey() . $i));
                                $dados_rel_quantidade["CODPRODUTO"] = $dados["CODPRODUTO"];

                                $model->produtos_rel_quantidade($dados_rel_quantidade);

                                $qntdd["CODQUANTIDADE"] = $dados_rel_quantidade["CODQUANTIDADE"];
                                $qntdd["QUANTIDADE"] = $_POST[$quantidade];
                                $qntdd["ROTULO"] = $_POST[$rotulo];
                                $qntdd["ESPECIFICACAO"] = $_POST[$especificacao];
                                $qntdd["IS_COR"] = ($_POST[$is_cor] == "on") ? 1 : 0;
                                $qntdd["COR"] = $_POST[$cor];
                                $qntdd["ORDEM"] = $_POST[$ordem];
                                $qntdd["STATUS"] = 1;


                                $model->insert_quantidade($qntdd);
                            }
                            $i++;
                        }

                        echo "<script>window.location='" . MEU_SITE . "produtos/produtos-lista'</script>";
                    }
    ...
    public function editar_get_dados() {
        if ($this->permitir_acesso($_SESSION["EMAIL"], $_SESSION["SENHA"], "ADMINISTRADOR")) {
            ...
            "<div class='btn btn-primary btn-lg' onclick=\"javascript:pTextAreaUpdateSubmit('pFromSubmit_{$produto->CODPRODUTO}','{$produto->CODPRODUTO}','/pt/produtos/editar-textarea/{$produto->CODPRODUTO}')\">Atualizar</div>" .
            "</div>" .
            "</td>" .
            "</tr>" .
            "</tbody>" .
    ...
    public function editar() {
        if ($this->permitir_acesso($_SESSION["EMAIL"], $_SESSION["SENHA"], "ADMINISTRADOR")) {
            ...
            $rotulos = $model->select_rotulo($key);
            ...
            if ($_POST) {
                ...
                if ($de == "") {
                    ...
                    $model->delete_rotulo($produto->URL_AMIGAVEL);
                    ...
                    if ($model->update_produto($dados, $produto->CODPRODUTO)) {
                        ...
                        while ($i < 101) {
                            ...
                            if ($_POST[$rotulo] != "") {
                                ...
                                $dados_rel_quantidade["CODQUANTIDADE"] = strtoupper(md5($this->getPrimarykey() . $i));
                                $dados_rel_quantidade["CODPRODUTO"] = $produto->CODPRODUTO;
...
//            $pessoa->STATUS] => 1 )
            ...
            $dadosboleto["demonstrativo3"] = "Estilo Carioca - https://www.estilocarioca.com.br";

            // INSTRUÇÕES PARA O CAIXA
            $dadosboleto["instrucoes1"] = "- Sr. Caixa, cobrar multa de 2% após o vencimento;";
            $dadosboleto["instrucoes2"] = "- Receber até 10 dias após o vencimento;";
            $dadosboleto["instrucoes3"] = "- Em caso de dúvidas entre em contato conosco: contato@estilocarioca.com.br;";
            $dadosboleto["instrucoes4"] = "- Pagável preferêncialmente nas Casas Lotéricas, Internet Bank ou Agências Santander;";
            ...
            $dadosboleto["codigo_cliente"] = "5703244-9"; // Código do Cliente (PSK) (Somente 7 digitos)
            ...
            $dadosboleto["endereco"] = "Rua Gazzi de Sá, nº17 - Lote 17 Quadra G , Barra da Tijuca";
            $dadosboleto["cidade_uf"] = "Rio de Janeiro / RJ";
            $dadosboleto["cedente"] = "RMGSPORT COMERCIO EIRELI - ME";

            // NÃO ALTERAR!
            //include("boletophp-master/include/funcoes_cef_sigcb.php");
            include("boletophp-master/include/funcoes_santander_banespa.php");

            if (sizeof($grava_boleto) > 0) {
                $model->insert_boletos($grava_boleto);
                $dnm["NOSSO_NUMERO"] = $nossonumero;
                $model->update_nosso_numero($dnm, $codpedido);
            }

            //include("boletophp-master/include/layout_cef.php");
            include("boletophp-master/include/layout_santander_banespa.php");
        }
    }
...
//        exit();
        ...
        if ($_POST["actionType"] == "criar_conta") {
            ...
            if (!preg_match('/^[^0-9][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/', $email)) {
                ...
                $emailsender = EMAIL_DISPATCHER;
                $nomeremetente = TITLE_LOJA;
                $emaildesitnatario = EMAIL_DISPATCHER;
                $comcopia = $email;
                ...
                $this->disparar_emailGoogleAppsHTML($emaildesitnatario, FALSE, $emailsender, $assunto, $mensagemHTML, $comcopia);
...
//                        mail($emaildesitnatario, $assunto, $mensagemHTML, $headers, "-f" . $emailsender);
    ...
    public function update_ordem() {

        $model = new Produtos_Model();

        $codproduto = $_POST["codproduto"];
        $dados["ORDEM"] = strtoupper($_POST["valor"]);

        $model->update_produto($dados, $codproduto);
        exit();
    }
    
    public function update_ordem_foto() {

        $model = new Produtos_Model();

        $codproduto = $_POST["codfoto"];
        $dados["ORDEM"] = strtoupper($_POST["valor"]);

        $model->update_foto($dados, $codproduto);
        exit();
    }
    ...
    public function update_qntdd_estoque() {
        ...
        $model->update_rotulo($dados, $codquantidade);
        exit();
    }
    ...
    public function sugestao() {
        ...
        $str = "";
        if ($sugestao) {
            $str .= "<div  style='padding: 2% 0 0; margin-top:-20px; background: #f4f4f4'>";
            $str .= "<p style='margin:5px 0 -10px 10px'>Sugestões para <b>{$search}</b></p><hr>";
            foreach ($sugestao as $obj) {
                $obj->NOME = str_replace($search, "<b>{$search}</b>", $obj->NOME);
                $str .= "<div class='sugestao'><a   href='" . MEU_SITE . "{$obj->CATEGORIA}/{$obj->URL_AMIGAVEL}'>{$obj->NOME}</a></div>";
            }
            $str .= "</div>";
        }
